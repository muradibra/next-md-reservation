        ...
        <ul className="flex flex-col items-start p-4">
          {navItemsWithoutPages.map((item) => (
            <li key={item.label} className="py-2">
              <Link
                href={item.href}
                className="text-black inline-block py-[10px] px-[20px] text-[16px]"
              >
                {item.label}
              </Link>
            </li>
          ...
          <li className="py-2 w-full">
            <span
              ...
              className="flex justify-between  items-center gap-3 text-black cursor-pointer py-[10px] px-[20px]"
            >
              Pages{" "}
              <ChevronDown
